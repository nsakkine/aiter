                       ...
                       bool q128kv64)
{
    ...
    ck_tile::stream_config stream_config{stream};
    float t = aiter::fmha_fwd_v3_fp8_sparse(a, stream_config, q128kv64);
    TORCH_CHECK(t >= 0, "fmha_v3_fwd_fp8_sparse: dispatcher returned an error code "
                        "(unsupported config or .co not found).");
    return {out};
}

// Persistent (grid-stride) fp8 sparse sibling. Identical data contract to
// fmha_v3_fwd_fp8_sparse plus a `work_table` int32[total_tiles] tensor whose
// entry k = packed q | (h<<16) | (b<<24) for the k-th tile to process (the
// caller LPT-sorts it by lut_count descending). Routes to the persistent .co.
std::vector<at::Tensor>
